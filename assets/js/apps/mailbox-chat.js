$(document).ready(function(){
  ...
  db.ref('chat').on('child_added', function(data){
        console.log("PRIMARY KEY: "+data.key);
        console.log(data.val());
        var personas = data.val().personas;

        var nombre = personas[0]['nombre'];
        $messageHtml = '<div class="person" data-value="'+data.key+'"><div class="user-info">';
        $messageHtml+= '<div class="f-head"><img src="assets/img/90x90.jpg" alt="avatar"></div><div class="f-body">';
        $messageHtml+= '<div class="meta-info"><span class="user-name" data-name="'+nombre+'">'+nombre+'</span><span class="user-meta-time">2:09 PM</span></div>';
        $messageHtml+= '<span class="preview lastMessage'+data.key+'"></span></div></div></div>';
        var appendMessage = $('.people').prepend($messageHtml);
    });
});
...
$('.user-list-box').on('click', '.person', function(event) {
    if ($(this).hasClass('.active')) {
        ...
        $('.chat').html("");
        chatValue = $(this).attr('data-value');
        ...
        /*FIREBASE*/
        var db = getDatabase();
        db.ref('chat/'+chatValue+'/mensajes').off('child_added');
        db.ref('chat/'+chatValue+'/mensajes').on('child_added', function(data){
          var chatId = data.ref.parent.parent.key;  //ID DEL CHAT

          if(chatId != chatValue){
            $(".lastMessage"+chatId).html(data.val().mensaje);
            console.log("AUMENTAR UN NO LEIDO");
            return;
          }

          var bubbleClass="you";
          if(data.val().id_persona == 1){bubbleClass="me";}

          $messageHtml = '<div class="bubble '+bubbleClass+'">' + data.val().mensaje + '</div>';
          var appendMessage = $('.chat').append($messageHtml);
        });
    }
...
$('.mail-write-box').on('keydown', function(event) {
    if(event.key === 'Enter') {
        if(chatValue == ''){
          alert('Debe escoger un chat'); return;
        }
        ...
        //sendMessage(chatMessageValue);
        var fecha = new Date().toDateString();
        db.ref('chat/'+chatValue+'/mensajes').push({
          fecha: fecha,
          id_persona: 1,
          mensaje: chatMessageValue
        });
        console.log("AGREGA NUEVO MENSAJE");
    }
})
