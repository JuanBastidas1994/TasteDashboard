let ApiUrl = "https://api.mie-commerce.com/gestion-errores/v1";
...
function initConfigGestionOrdenes(){
    getConfigInicial()
        .then(data => {
            ...
            //Indicadores
            $("#preordenesErrors").html(data.errores.preordenesErrors);
            $("#ordenesCanceledByUser").html(data.errores.ordenesCanceledByUser);
            $("#ordenesCanceledByCourier").html(data.errores.ordenesCanceledByCourier);
            $("#ordenesErrorAsignacion").html(data.errores.ordenesErrorAsignacion);
            $("#ordenesErrorFacturacion").html(data.errores.ordenesErrorFacturacion);
...
function preAsignarOrden(order_id, courier_id, name){
...
function openLinkModal(order_id){
    SwalText("Generar Link para motorizado", "Proporcionados el número teléfonico del motorizado", "Teléfono (+593)")
        .then(function(result) {
            if (result) {
