...
$(function() {    
    ...
    db.ref(firebaseColumn).limitToLast(1).on('child_added', function(data){
        if(showNewOrdersFirebase){
            ...
            if(parseInt(orden.auto_asignar) == 1)
                autoAsignarOrden(orden.id, orden.envio);
    ...
    db.ref(firebaseColumn).limitToLast(1).on('child_changed', function(data){
        ...
        if(orden.estado === "ENTREGADA"){
            successSoundPlay();
            calcularPuntosUsuarioByOrden(orden.id);
            facturar_inventario(orden.id, 'ENTREGADA');
...
//Status User Online
function setUserToFirebase(_status){
    let date = moment().format('YYYY-MM-DD HH:mm:ss');
    let config = getConfigGestionOrdenes();
    db.ref('usuarios/'+casherId).set({
        id: casherId,
        business: $("#alias_empresa").val(),
        office: config.sucursal.nombre,
        date: date,
        status: _status
    });
}