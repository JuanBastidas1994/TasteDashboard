    ...
    fetch(`${ApiUrl}/ordenes`,{
        ...
        .then(response => {
            ...
            if(response.success == 1){
                ...
                $("#lista_ordenes").html(template(response.data));
                feather.replace();
            }else{
                let text = "";
...
function preAsignarOrden(order_id, courier_id, name){
    if(courier_id == 99){
        ...
        /*
        let options = [];
        config.motorizados.forEach(motorizado => {
            options[motorizado.id] = motorizado.nombres + " Tel." + motorizado.telefono;
        });
        SwalSelect("Escoja el motorizado a asignar", options)
                        .then(function(result) {
                            if (result) {
                                console.log("result", result);
                                asignarOrden(order_id, courier_id, result)
                            }
                        });*/
    }else{
    ...
    fetch(`${ApiUrl}/ordenes/asignar`,{
        ...
        .then(response => {
            ...
            if(response.success == 1){
                ...
                // sendNotify("usuarios", "usuario", "Test Title", "Test Body", "USUARIOS", 9308);
                // sendNotify("motorizados", "motorizado", "Test Title", "Test Body", "MOTORIZADOS", 13364);
                facturaElectronica(order_id, true); //SI ES FALSE ES ANULAR
            }else{
    ...
    fetch(`${ApiUrl}/ordenes/cancelar-courier`,{
        ...
        .then(response => {
            ...
            if(response.success == 1){
                ...
                openOrden(order_id);
                notify(response.mensaje, "success", 2);
                facturaElectronica(order_id, false); //SI ES FALSE ES ANULAR
            }else{
                messageDone(response.mensaje,'error');
            }
        })
