...
function getClient(cod_usuario) {
    OpenLoad("Cargando...");
    fetch(`controllers/controlador_clientes.php?metodo=fidelizacion&cod_usuario=${cod_usuario}`,{
        method: 'GET',
    })
    .then(res => res.json())
    .then(response => {
...
function getOtherCredits(cod_usuario) {
    OpenLoad("Cargando...");
    fetch(`controllers/controlador_clientes.php?metodo=otherCredits&cod_usuario=${cod_usuario}`,{
        method: 'GET',
    })
    .then(res => res.json())
    .then(response => {
        ...
        console.log(response);
        if(response.success == 1){
            var target = $("#table-other-credits");
            var template = Handlebars.compile($("#other-credits-template").html());
