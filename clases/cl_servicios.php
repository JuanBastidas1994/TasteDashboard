...
class cl_servicios
{
	...
	var $session;
	var $cod_producto, $cod_producto_padre, $cod_empresa, $alias, $nombre, $desc_corta, $desc_larga, $image_min, $image_max, $fecha_create, $user_create, $estado, $codigo, $categorias, $open_detalle, $peso,$sku,$cobra_iva,$is_combo, $facturar_sin_stock;
	var $precio, $precio_no_tax, $iva_valor, $iva_porcentaje, $precio_anterior, $costo;
	var $nomOpcion, $precio_min, $precio_max, $isCheck, $isDatabase, $cod_producto_opcion, $nomDetalle, $precioDet, $aumentar_precio, $posicion, $intervalo;
	...
	public function crear(&$id){
		...
		$empresa = $this->session['cod_empresa'];
		$query = "INSERT INTO tb_productos(cod_producto_padre, cod_empresa, alias, nombre, desc_corta, desc_larga, image_min, image_max, fecha_create, user_create, estado, precio, precio_no_tax, iva_valor, iva_porcentaje, codigo, precio_anterior, costo, open_detalle, peso,sku,is_combo,cobra_iva, noStock, intervalo) ";
		$query.= "VALUES($this->cod_producto_padre, $empresa, '$this->alias', '$this->nombre', '$this->desc_corta', '$this->desc_larga', '$this->image_min', '$this->image_max', NOW(), $usuario, 'A', $this->precio, $this->precio_no_tax, $this->iva_valor, $this->iva_porcentaje, '$this->codigo', $this->precio_anterior, $this->costo, $this->open_detalle, '$this->peso', '$this->sku', $this->is_combo, $this->cobra_iva, $this->facturar_sin_stock, '$this->intervalo')";
	...
	public function editar(){
		$usuario = $this->session['cod_usuario'];
		$empresa = $this->session['cod_empresa'];
		$query= "UPDATE tb_productos SET cod_producto_padre=$this->cod_producto_padre, nombre='$this->nombre', desc_corta='$this->desc_corta', desc_larga='$this->desc_larga', estado='$this->estado', precio=$this->precio, precio_no_tax=$this->precio_no_tax, iva_valor=$this->iva_valor, iva_porcentaje=$this->iva_porcentaje, codigo='$this->codigo',precio_anterior=$this->precio_anterior, costo= $this->costo , open_detalle= $this->open_detalle, peso = '$this->peso',sku='$this->sku',is_combo='$this->is_combo',cobra_iva='$this->cobra_iva', noStock='$this->facturar_sin_stock',fecha_modificacion=NOW(), intervalo='$this->intervalo' WHERE cod_producto = $this->cod_producto";
		if(Conexion::ejecutar($query,NULL)){
	...
	public function crear_opcion($cod_producto, $titulo, $cantidad, $cantidad_max, $productos, $tipo_opcion, $cmb_isCheck, &$id){
		$productos = array_map('htmlentities', $productos);
