...
function asignarPicker(){
    ...
    extract($_POST);
    $orden = $Clordenes->get_orden_array($id);
    $picker = $Clsucursales->getPickerSucursal($orden['cod_sucursal']);
    if($picker){
        require_once "../clases/cl_picker.php";
        $ClPicker = new cl_picker($orden['cod_sucursal']);
        $respPicker = $ClPicker->crearOrder($orden);
        if(isset($respPicker->data)){
            $idPicker = $respPicker->data->bookingNumericId;
            $return['idPicker']=$idPicker;
            $cod_courier = 3; // TIPO PICKER
            $isGacela=$Clordenes->order_updCourier($id,$idPicker,$cod_courier);
            if($isGacela){
                 $return['success'] = 1;
                $return['mensaje'] = "Orden asignada correctamente";
            }
                ...
                $return['success'] = 0;
                $return['mensaje'] = "Error al definir la orden en picker, por favor vuelva a intentarlo";
            } 
        }else{
            $return['success'] = 0;
            $return['mensaje'] = "Error al crear la orden en Picker, por favor vuelva a intentarlo";
            $return['respPicker'] = $respPicker;
        }    
    }else{
        $return['success'] = 0;
        $return['mensaje'] = "En esta sucursal no está configurado picker, por favor revisar";
    }
...
function asignarInlog(){
    ...
    $ch = curl_init("https://api.mie-commerce.com/taste/v1/ordenes/asignar");
    $headers = array();
    $headers[] = 'Content-Type: application/json';
    $headers[] = 'Api-Key: API-healthy-beauty--1WYAWR7OE1UNBBW';
    ...
    if ($response === false) {
        $return['success'] = 0;
        $return['mensaje'] = "Curl error: " . curl_error($ch);
        return $return;
    }
    curl_close($ch);
...
function get_orden(){
    ...
    if($orden){
        ...
        $orden['is_programado'] == 1 ? $fecha = $orden['hora_retiro'] : $fecha = $orden['fecha'];
        $fecha = explode(" ", $fecha);
        $return['fecha'] = $fecha[0];
        //$return['htmlV2'] = itemOrdenExpandidoV2($orden);
    }else{
        $return['success'] = 0;
        $return['mensaje'] = "No existe la orden con el id $id";
    }
    return $return;
}
  ...
  if(!isset($_GET['cod_orden'])){
    ...
    if($pagoTarjeta){
        ...
        if($pagoTarjeta['proveedor'] == 1){ //DATAFAST
            ...
            $return['respDatafast'] = $refund;
            
            $json = base64_encode(json_encode($refund));
...
function getInfoPaymentez(){
    ...
    if($pagoTarjeta){
         $mensaje = "";
         $codigo = $pagoTarjeta['observacion'];
         $transaction = getTransaction($codigo, $cod_orden, $mensaje);
         if($transaction){
          $return['success'] = 1;
          $return['mensaje'] = $mensaje;
...
function recordarOrdenPendiente(){
    global $Clordenes;
    global $Clsucursales;
    global $session;
    extract($_GET);
    ...
    $ordenes = $Clordenes->recordarOrdenPendiente($cod_empresa, $tiempo);

    if($ordenes){
        for ($i=0; $i<count($ordenes); $i++) { 
            $couriers = $Clsucursales->getCouriers($ordenes[$i]['cod_sucursal']);
            if($couriers){
                $ordenes[$i]['courier'] = $couriers[0]['cod_courier'];
                $ordenes[$i]['textCourier'] = $couriers[0]['nombre'];
            }
            else{
                $ordenes[$i]['courier'] = 0;
                $ordenes[$i]['textCourier'] = "";
            }
        }
        $return['success'] = 1;
        $return['mensaje'] = "Hay órdenes ".$fecha .$ordenes['cod_sucursal'];
        $return['ordenes'] = $ordenes;
    }
    else{
        $return['success'] = 0;
        $return['mensaje'] = "No hay órdenes ".$fecha;
    }
    return $return;
}

function setPermisoRecordatorio(){
    global $Clempresas;
    global $session;
    $cod_empresa = $session['cod_empresa'];
    extract($_GET);
    ...
    $act = "desactivado";
    if($recordar_ordenes == 1)
        $act = "activado";

    if($Clempresas->setPermisoRecordatorio($cod_empresa, $recordar_ordenes)){
        $return['success'] = 1;
        $return['mensaje'] = "Recordar órdenes: ".$act;
    }
    else{
        $return['success'] = 0;
        $return['mensaje'] = "Error al actualizar el permiso";
    }
    return $return;
}
...
function getOrdenesRunfood(){
    global $Clordenes;
    global $session;
    $ordenes = $Clordenes->getOrdenesRunfood($session['cod_empresa']);
    if($ordenes){
        $return['data'] = $ordenes;
        $return['success'] = 1;
        ...
        $return['success'] = 0;
        $return['mensaje'] = "No hay lista de órdenes ".$session['cod_empresa'];
    }
    return $return;
}
?>